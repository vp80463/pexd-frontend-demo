...
const queryFormData = reactive({
  targetMonth: '', productCd: '', productCd: '', productCd: '',
});
...
const gridAdjustmentPlusAmt12Formatter = (row, columnConfig, cellValue) => {
  return formatAmount(row.cellValue);
};
const gridAdjustmentPlusAmt12EditRender = computed(() => {
  return {
    enabled: false,
    attrs: {
      textAlign: 'right',
...
const gridAdjustmentPlusAmt53Formatter = (row, columnConfig, cellValue) => {
  return formatAmount(row.cellValue);
};
const gridAdjustmentPlusAmt53EditRender = computed(() => {
  return {
    enabled: false,
    attrs: {
      textAlign: 'right',
      ...
      >
        <VuePanel id="viy2Panel_7m7hlM" ref="viy2Panel_7m7hlM" title="明細情報" collapse-icon-position="left" height="100%">
          ...
          <VueTable id="grid" ref="grid" header-align="center" height="auto" :data="findStockAccountList" :edit-config="gridEditConfig" :mouse-config="gridMouseConfig" @cell-dblclick="gridCellDblclick">
            ...
            />
            <VueNumberColumn
              :formatter="gridAdjustmentPlusAmt12Formatter"
              :edit-render="gridAdjustmentPlusAmt12EditRender"
              field="adjustmentPlusAmt12"
              align="right"
              aggregate="sum"
              footer-align="right"
              :sortable="true"
            ...
            />
            <VueNumberColumn
              :formatter="gridAdjustmentPlusAmt53Formatter"
              :edit-render="gridAdjustmentPlusAmt53EditRender"
              field="adjustmentPlusAmt53"
              align="right"
              aggregate="sum"
              footer-align="right"
              :sortable="true"
